...
export default function ElementalTrialsPage() {
  ...
  const fetchLeaderboard = useCallback(async () => {
    setLoadingLB(true)
    try {
      const res = await fetch('/api/leaderboard?game=elemental-trials&limit=15')
  ...
  useEffect(() => {
    function onMessage(e: MessageEvent) {
      if (e.data?.type === 'elemental-game-over') {
  ...
  return (
    <div className="flex flex-col" style={{ height: '100dvh', background: '#080e14' }}>
      ...
      <div
        ...
        style={{ background: 'rgba(8,14,20,0.95)', borderBottom: '1px solid rgba(255,107,107,0.25)' }}
      >
        <Link
          href="/"
          className="font-pixel flex items-center gap-2 no-underline"
          style={{ fontSize: '9px', color: 'var(--coral)', letterSpacing: '1px' }}
        >
          ◄ ARCADE
        </Link>

        <div className="font-pixel text-center" style={{ fontSize: 'clamp(8px,1.5vw,11px)', color: '#ff9d9d', letterSpacing: '2px' }}>
          ELEMENTAL TRIALS
        </div>
      ...
      <div className="flex-1 relative">
        <iframe
          ref={iframeRef}
          src="/games/elemental-trials.html"
      ...
      {showScoreModal && gameResult && (
        ...
        >
          <div
            ...
            style={{ background: '#0d1b2a', border: '2px solid rgba(255,107,107,0.5)', boxShadow: '0 0 40px rgba(255,107,107,0.2)' }}
          >
            {!submitted ? (
              <>
                <div className="font-pixel text-center mb-1" style={{ fontSize: '11px', color: '#ff9d9d', letterSpacing: '2px' }}>
                  {gameResult.reason === 'win' ? 'ALL GUARDIANS FELL' : 'YOU FELL'}
                </div>
                ...
                <div
                  ...
                  style={{ background: 'rgba(8,14,20,0.8)', border: '1px solid rgba(255,107,107,0.2)' }}
                >
                  <StatCell label="SCORE" value={String(gameResult.score)} />
                  <StatCell label="BOSSES" value={String(gameResult.kills)} />
                  <StatCell label="STAGE" value={String(gameResult.level)} />
                </div>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-3">
                  ...
                  <button
                    ...
                    style={{
                      fontSize: '10px',
                      color: '#080e14',
                      background: 'linear-gradient(180deg, #ff9d9d, var(--coral))',
              ...
              <div className="text-center">
                ...
                <div className="flex gap-3 justify-center">
                  ...
                  <button
                    onClick={() => setShowScoreModal(false)}
                    className="font-pixel rounded px-4 py-2 cursor-pointer"
                    style={{ fontSize: '8px', color: 'var(--coral)', background: 'rgba(255,107,107,0.1)', border: '1px solid rgba(255,107,107,0.4)' }}
                  >
                    KEEP PLAYING
                  </button>
                </div>
          ...
          >
            ...
            <div className="overflow-y-auto" style={{ maxHeight: '420px' }}>
              {loadingLB ? (
                ...
                <div className="text-center py-12">
                  ...
                  <div className="font-vt" style={{ fontSize: '18px', color: 'var(--coral)', opacity: 0.7 }}>Be the first to submit!</div>
                </div>
              ) : (
                <table className="w-full">
                  <thead>
                    <tr style={{ borderBottom: '1px solid rgba(255,209,102,0.2)' }}>
                      {['#', 'PLAYER', 'SCORE', 'BOSSES', 'STAGE'].map(h => (
                  ...
                  <tbody>
                    {leaderboard.map((entry, i) => (
                      ...
                      >
                        ...
                        <td className="font-pixel px-4 py-3"
                          style={{ fontSize: '9px', color: '#ff9d9d' }}>
                          {entry.score}
                        </td>
                        <td className="font-pixel px-4 py-3"
            ...
            >
              ...
              <button
                ...
                className="font-pixel cursor-pointer"
                style={{ fontSize: '7px', color: 'var(--coral)', background: 'none', border: 'none' }}
              >
                ↻ REFRESH
              </button>
            </div>
...
function StatCell({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <div className="font-pixel" style={{ fontSize: '14px', color: '#ff9d9d' }}>{value}</div>
